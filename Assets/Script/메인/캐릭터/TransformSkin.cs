    ...
    public void UserEqipInfoSetting()
    {
        ...
        for (int i = 0; i < GameManager.instance.userInfoManager.skinItem.Count; i++)
        {
            if (GameManager.instance.userInfoManager.skinItem[i].isEqip)
            {
                ...
                // 해당 캐릭터와 아이템장착 가능한 캐릭터가 똑같을 때 만 장착
                if (!skinName.Contains("Body"))
                {
                    if (GameManager.instance.itemManager.GetItemInfo(skinName).characterName.Contains(GameManager.instance.userInfoManager.currentCharacter))
                    {
                        SkinChange(skinName);
